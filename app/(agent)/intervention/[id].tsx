...
import { getCurrentCoordinates, verifyProximityToSite } from '../../../src/utils/location';
import { checkIn, checkOut, heartbeat, markArrival } from '../../../src/services/api/attendance.api';
import { AnomalyType } from '../../../src/types/anomaly';
import { createAnomaly } from '../../../src/services/api/anomalies.api';
...
export default function InterventionDetailScreen() {
  ...
  }, [intervention?.actualStartAt, intervention?.actualStartTime, intervention?.date]);

  const hasPendingSync = intervention
    ? pendingEvents.some((event) => event.interventionId === intervention.id)
    : false;
    ...
    <HeaderLayout
      ...
      trailing={
        <Button
          ...
          disabled={isMarkingArrival || isVerifying || arrivalRecorded}
          size="sm"
        />
      }
    >
        ...
        {isRunning && (
          <Section title="Checklist">
            <Text style={styles.textMuted}>
              Checklist à venir. Veillez à suivre les consignes de votre superviseur.
            </Text>
          </Section>
        )}
      </ScrollView>
      ...
      <ProblemModal
        ...
        onSubmit={async () => {
          ...
          try {
            ...
            Alert.alert('Merci', 'Votre anomalie a été transmise à votre superviseur.');
...
function getActualDate(intervention: Intervention) {
  ...
  if (intervention.actualStartTime) {
    return new Date(`${intervention.date}T${intervention.actualStartTime}:00`);
  }
  return null;
}
